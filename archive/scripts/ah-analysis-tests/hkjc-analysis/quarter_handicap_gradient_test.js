...
const quarterBettingStrategies = {
    ...
    'Kelly': {
        name: 'Kelly-Style for Quarters',
        description: 'Kelly betting assuming 8% edge in quarter handicaps',
        calculateStake: (odds) => {
            const impliedProb = 1 / odds;
            const assumedTrueProb = impliedProb + 0.08; // Assume 8% edge in quarters
            const edge = assumedTrueProb - impliedProb;
            const kellyFraction = edge / (odds - 1);
            return Math.max(Math.min(kellyFraction * 1500, 500), 30); // Scale for quarters
        }
    }
};
...
function testQuarterGradientBetting() {
    ...
    console.log('Match                              | Handicap   | Higher | Odds | Fixed | Linear| Trapped| Result   | Profit');
    ...
    allMatches.forEach(match => {
        ...
        Object.keys(quarterBettingStrategies).forEach(strategyKey => {
            ...
            // Store quarter case data
            results[strategyKey].quarterCases.push({
                handicap: ah.homeHandicap,
                odds: higherOdds,
                extreme: isExtremeCase,
                result: betResult,
                profit: profit
            });
            ...
            if (sampleBetsShown < 20 && strategyKey === 'Fixed') {
                ...
                console.log(`${matchStr.padEnd(34)} | ${ah.homeHandicap.padEnd(10)} | ${sideStr.padEnd(6)} | ${higherOdds.toFixed(2).padStart(4)} | ${fixedStake.toFixed(0).padStart(5)} | ${linearStake.toFixed(0).padStart(5)} | ${trappedStake.toFixed(0).padStart(6)} | ${resultCode.padEnd(8)} | ${profit.toFixed(0).padStart(6)}`);
                sampleBetsShown++;
            }
        });
    });
...
// Analyze by handicap patterns
function analyzeByHandicapPattern(results) {
    console.log('\n🔍 ANALYSIS BY QUARTER HANDICAP PATTERNS:');
    console.log('='.repeat(70));
    
    // Get all quarter cases from Fixed strategy (they're all the same)
    const quarterCases = results['Fixed'].quarterCases;
    
    // Group by handicap pattern
    const patterns = {};
    quarterCases.forEach(case_ => {
        if (!patterns[case_.handicap]) {
            patterns[case_.handicap] = {
                cases: 0,
                totalProfit: 0,
                wins: 0,
                extremeCases: 0
        ...
        patterns[case_.handicap].cases++;
        patterns[case_.handicap].totalProfit += case_.profit;
        if (case_.result === 'win' || case_.result === 'half-win') {
            patterns[case_.handicap].wins++;
        }
        if (case_.extreme) {
            patterns[case_.handicap].extremeCases++;
        }
    });
    
    console.log('Handicap      | Cases | Win Rate | Extreme% | ROI    | Trapped Effect');
    console.log('-'.repeat(70));
    
    Object.entries(patterns)
        .sort(([,a], [,b]) => b.cases - a.cases)
        .forEach(([handicap, data]) => {
            const winRate = (data.wins / data.cases * 100);
            const roi = (data.totalProfit / (data.cases * 100) * 100);
            const extremePercent = (data.extremeCases / data.cases * 100);
            const trappedEffect = extremePercent > 50 ? '🔥 HIGH' : extremePercent > 20 ? '⚡ MED' : '😐 LOW';
            
            console.log(`${handicap.padEnd(13)} | ${data.cases.toString().padStart(5)} | ${winRate.toFixed(1).padStart(8)} | ${extremePercent.toFixed(1).padStart(8)} | ${roi.toFixed(1).padStart(6)} | ${trappedEffect}`);
        });
}
...
analyzeByHandicapPattern(results);

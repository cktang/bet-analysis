...
function calculateProfit(betChoice, match, betAmount = 100) {
    ...
    // Extract match details properly
    const matchData = match.match || match;
    const homeScore = matchData.homeScore || match.fbref?.homeGoals || 0;
    const awayScore = matchData.awayScore || match.fbref?.awayGoals || 0;
    
    // Get Asian Handicap odds and handicap
    const ah = matchData.asianHandicapOdds || {};
    const homeOdds = ah.homeOdds || 1.9;
    const awayOdds = ah.awayOdds || 1.9;
    
    // Parse handicap (could be "0/+0.5" format)
    let handicap = 0;
    if (ah.homeHandicap) {
        const handicapStr = ah.homeHandicap.toString();
        if (handicapStr.includes('/')) {
            // Handle split handicap like "0/+0.5"
            const parts = handicapStr.split('/');
            handicap = (parseFloat(parts[0]) + parseFloat(parts[1])) / 2;
        } else {
            handicap = parseFloat(handicapStr) || 0;
        }
    }
...
winningStrategies.forEach((strategy, index) => {
    ...
    matchData.forEach(match => {
        try {
            ...
            if (betChoice) {
                ...
                const homeTeam = matchDetails.homeTeam || 'Unknown';
                const awayTeam = matchDetails.awayTeam || 'Unknown';
                const homeScore = matchDetails.homeScore || match.fbref?.homeGoals || 0;
                const awayScore = matchDetails.awayScore || match.fbref?.awayGoals || 0;
                const date = match.fbref?.date || matchDetails.date || 'Unknown';
                
                const ah = matchDetails.asianHandicapOdds || {};
                const homeOdds = ah.homeOdds || 1.9;
                const awayOdds = ah.awayOdds || 1.9;
    ...
    console.log(`  Generated ${totalBets} betting records with ${totalBets > 0 ? (totalProfit / (totalBets * 100) * 100).toFixed(2) : '0'}% profitability`);
});
...
// Generate README
const readmeContent = `# Winning Strategies Betting Records (FIXED)

This folder contains CORRECTED detailed betting records for all ${winningStrategies.length} profitable strategies discovered in our Asian Handicap analysis.
...
- **Fixed Issues**: Proper team names, scores, odds, and profit calculations

...
describe('BettingExecutorService Direct Tests', () => {
  ...
  beforeEach(() => {
    ...
    service = new BettingExecutorService(mockConfigService, mockDataFileService);
  });

  afterEach(async () => {
    // Clean up if needed
    if (service && typeof (service as any).closeBrowserSession === 'function') {
      await (service as any).closeBrowserSession();
    }
  });
  ...
  describe('Service Initialization', () => {
    it('should be defined', () => {
      ...
      expect(service.constructor.name).toBe('BettingExecutorService');
    });

    it('should have dependencies injected', () => {
      expect((service as any).configService).toBeDefined();
      expect((service as any).dataFileService).toBeDefined();
      expect((service as any).configService.get).toBeDefined();
    });
  ...
  describe('Configuration Tests', () => {
    ...
    it('should call config service methods', () => {
      service.getExecutorStatus();
      
      expect(mockConfigService.get).toHaveBeenCalledWith('ENABLE_LIVE_BETTING', false);
      expect(mockConfigService.get).toHaveBeenCalledWith('ENABLE_PAPER_TRADING', true);
      expect(mockConfigService.get).toHaveBeenCalledWith('HEADLESS_BROWSER', true);
    });
  });
